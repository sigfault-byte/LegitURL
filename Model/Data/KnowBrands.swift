//
//  KnownBrands.swift
//  LegitURL
//
//  Created by Chief Hakka on 07/03/2025.
//
struct KnownBrands {
    static let names: Set<String> = [
        // 🔹 Tech Giants & Software
        "apple", "microsoft", "google", "amazon", "openai", "adobe", "oracle", "ibm",
        "intel", "nvidia", "amd", "samsung", "huawei", "lenovo", "dell", "hp",
        
        // 🔹 Social Media & Communication
        "facebook", "twitter", "instagram", "linkedin", "whatsapp", "tiktok",
        "snapchat", "telegram", "discord", "reddit", "pinterest", "quora", "weibo",
        
        // 🔹 Video & Streaming
        "youtube", "netflix", "spotify", "twitch", "disney", "hulu", "primevideo",
        "paramount", "hbo", "peacock", "applemusic", "soundcloud", "deezer",
        
        // 🔹 E-Commerce & Retail
        "ebay", "walmart", "target", "alibaba", "aliexpress", "rakuten", "shopify",
        "etsy", "bestbuy", "costco", "flipkart", "zalando", "shein",
        
        // 🔹 Finance & Banking
        "paypal", "visa", "mastercard", "stripe", "revolut", "wise", "americanexpress",
        "capitalone", "hsbc", "citibank", "wellsfargo", "chase", "bankofamerica",
        "monzo", "n26", "sofi", "goldmansachs",
        
        // 🔹 Crypto & Trading Platforms
        "binance", "coinbase", "kraken", "bitstamp", "bybit", "kucoin", "crypto",
        "etoro", "robinhood", "fidelity", "vanguard", "nasdaq", "forex",
        
        // 🔹 Travel & Transportation
        "uber", "lyft", "airbnb", "booking", "expedia", "tripadvisor", "trivago",
        "hotels", "delta", "emirates", "qatarairways", "ryanair", "easyjet",
        "boeing", "airbus", "tesla", "bmw", "mercedes", "toyota", "honda",
        "ford", "volkswagen", "nissan",
        
        // 🔹 Food & Beverages
        "mcdonalds", "starbucks", "burgerking", "dominos", "kfc", "subway",
        "chipotle", "pizza", "dunkindonuts", "pepsi", "cocacola", "nestle",
        
        // 🔹 Security & VPN
        "nordvpn", "expressvpn", "cyberghost", "surfshark", "bitdefender",
        "mcafee", "kaspersky", "sophos", "avast", "malwarebytes",
        
        // 🔹 Gaming & Consoles
        "steam","steampowered", "epicgames", "playstation", "xbox", "nintendo", "rockstargames",
        "blizzard", "ea", "riotgames", "bethesda", "squareenix", "ubisoft",
        
        // 🔹 Healthcare & Pharma
        "who", "cdc", "nih", "pfizer", "moderna", "johnsonandjohnson",
        "roche", "astrazeneca", "sanofi", "bayer", "novartis",
        
        // 🔹 Education & Research
        "harvard", "mit", "stanford", "cambridge", "oxford", "berkeley",
        "columbia", "princeton", "yale", "coursera", "udemy", "khanacademy",
        
        // 🔹 News & Media
        "bbc", "cnn", "nytimes", "forbes", "reuters", "bloomberg", "wsj",
        "guardian", "aljazeera", "lemonde", "washingtonpost", "time"
    ]
}
