    ...
    static func analyze(urlInfo: inout URLInfo, comp: String = "query") -> (URLInfo, String?) {
        ...
        for index in keys.indices {
            ...
            if let key = keys[index], !key.isEmpty {
                let keyNode = LamaiDecoder.decode(input: key, maxDepth: 4)
                ...
                if let url = NodeWalker.analyze(node: keyNode, urlInfo: &urlInfo, comp: "comp", label: "key"), !url.isEmpty{
                    foundURL.append(url)
                }
            }
...
//                deeper because value are supposedly more obfuscated
                let valueNode = LamaiDecoder.decode(input: value, maxDepth: 6)
                ...
                if let url = NodeWalker.analyze(node: valueNode, urlInfo: &urlInfo, comp: comp, label: "value"), !url.isEmpty{
                    foundURL.append(url)
                }
            }
        }
