...
struct HostAnalyser {
    static func analyze(urlObject: inout URLInfo) {
        ...
        if shouldSkipDomainAnalysis {
            ...
            DomainAnalyzer.analyze(in: &urlObject, domain: domainRoot, tld: tld)
            TLDAnalyzer.analyze(tld, urlInfo: &urlObject)
        }
        
        SubdomainAnalyzer.analyze(urlInfo: &urlObject, subdomains: subdomains)
